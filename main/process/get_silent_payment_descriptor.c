...
void get_silent_payment_descriptor_process(void* process_ptr)
{
    ...
    const size_t account_index = rpc_get_sizet_or("account", &params, 0);
    if (account_index >= SP_ACCOUNT_INDEX_MAX) {
        jade_process_reject_message(process, CBOR_RPC_BAD_PARAMETERS, "Account index too large");
        goto cleanup;
    }
