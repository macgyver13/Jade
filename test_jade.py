...
HmWPvgD3hiTnD5KZuMkxSUsgGraZ9vavB5JSA3F9s5E4cXuCte5rvBs5N4DjfxYssQk1L82Bq4FE"
    ...
    bad_params = [(('badauth1', 'auth_user'), 'Expecting parameters map'),
                  ...
                  (('baddescr16', 'register_descriptor',
                   ...
                   'Failed to parse descriptor'),
                  # An empty datavalues map is valid - a musig() or sp() descriptor
                  # names no signers - so this now fails on the descriptor itself
                  (('baddescr17', 'register_descriptor',
                    {'network': 'testnet', 'descriptor_name': 'too_few', 'descriptor': 'test',
                     'datavalues': {}}), 'Failed to parse descriptor'),
...
SP_375_REJECTIONS = {
    ...
    'resolved_output_modifiable': 'Silent payment outputs resolved while the transaction can still change',
...
def test_silent_payment_sign_psbt(jadeapi):
    testcase = next(_get_test_cases('psbt_sp_resolve_sign.json'), None)
    if testcase is None:
    ...
    # cleared or Jade refuses to sign.
