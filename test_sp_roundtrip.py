...
def run_collaborative_roundtrip(jade, network='localtest', account=0, verbose=True):
    ...
    # Setting the output scripts fixes the inputs and outputs, as BIP375
    # requires of the resolver: Jade refuses resolved outputs on a psbt that
    # can still change
    assert psbt.contents.tx_modifiable_flags & 0x03 == 0
...
def signer_indices(sp, *indices):
    return (sp.c_uint32 * len(indices))(*indices)
...
def cosigner_round1(sp, setup, data, entropy=b'\x33' * 64):
    ...
    ret, status = sp.wally_psbt_sp_musig_round1(
        psbt, sp.byref(musig_input(sp, setup)), 1, signer_indices(sp, 0), 1,
...
def cosigner_round2(sp, setup, data, secnonce, digest):
    ...
    assert sp.wally_psbt_sp_musig_round2(psbt, sp.byref(musig_input(sp, setup)), 1,
                                         signer_indices(sp, 0), 1, seckey, len(seckey), nonce, digest,
...
def run_musig_negative_cases(jade, sp, setup, recipient, network='localtest', log=print):
    ...
    # A BIP-376 input of Jade's that it cannot sign must be refused outright,
    # not left waiting for a share Jade will never add
    bad_spend = add_misplaced_spend_input(jade, sp, network,
                                          build_musig_psbt(sp, setup, recipient['info']))
    log(f'Rejected: {assert_rejected(jade, network, bad_spend, SP_SPEND_PATH_ERROR)}')


def add_misplaced_spend_input(jade, sp, network, data):
    """Add a BIP-376 input whose spend key is Jade's, but not a BIP-352 one."""
    fingerprint = get_fingerprint(jade, sp, network)
    (key,), _ = get_wallet_keys(jade, sp, network, 1, fingerprint)
    psbt = parse_psbt(sp, data)
    index = psbt.contents.num_inputs
    txid = bytes([0x6c] * 32)
    tx_input = sp.pointer(sp.wally_tx_input())
    assert sp.wally_tx_input_init_alloc(txid, len(txid), 0, 0xffffffff, None, 0, None,
                                        tx_input) == sp.WALLY_OK
    assert sp.wally_psbt_add_tx_input_at(psbt, index, 0, tx_input) == sp.WALLY_OK
    script = b'\x51\x20' + key['pubkey'][1:]
    utxo = sp.pointer(sp.wally_tx_output())
    assert sp.wally_tx_output_init_alloc(SP_AMOUNT, script, len(script), utxo) == sp.WALLY_OK
    assert sp.wally_psbt_set_input_witness_utxo(psbt, index, utxo) == sp.WALLY_OK
    assert sp.wally_psbt_set_input_amount(psbt, index, SP_AMOUNT) == sp.WALLY_OK
    tweak = bytes([0x01] * 32)
    assert sp.wally_psbt_set_input_sp_tweak(psbt, index, tweak, len(tweak)) == sp.WALLY_OK
    add_keypath(sp, psbt, index, key, sp.wally_psbt_add_input_sp_spend_keypath)
    return serialize_psbt(sp, psbt, free=True)


def run_musig_concurrent_sessions(jade, sp, setup, recipient, network='localtest', log=print):
    """Two transactions from the same MuSig2 wallet, interleaved.

    Both spend input 0 under the same aggregate, so only the session digest
    tells their nonces apart: starting the second must not expire the first.
    """
    first = bytes(jade.sign_psbt(network, build_musig_psbt(sp, setup, recipient['info'])))
    second = bytes(jade.sign_psbt(network, build_musig_psbt(
        sp, setup, recipient['info'], amount=MUSIG_SP_AMOUNT - 1000)))
    for round1 in (first, second):
        resolved, cosigner_nonce, status = cosigner_round1(sp, setup, round1)
        assert status == WALLY_SP_COMPLETE
        psbt = parse_psbt(sp, resolved)
        digest = session_digest(sp, psbt)
        sp.wally_psbt_free(psbt)
        round2 = bytes(jade.sign_psbt(network, resolved))
        verify_musig_signature(sp, setup, cosigner_round2(sp, setup, round2, cosigner_nonce, digest))
