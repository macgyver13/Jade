...
bool wallet_is_expected_sp_spend_path(const network_t network_id, const uint32_t* path, const size_t path_len)
{
    ...
    // Any account the descriptor export can name, ie. the uint16_t range the
    // rpc call and the qr export options offer. A narrower bound here would
    // leave payments received on an exported account unspendable.
    if (path[2] < SUBACT_FLOOR || path[2] > harden(UINT16_MAX)) {
        return false;
    }
